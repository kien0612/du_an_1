...
function loadAll_hoa_don(){
    $sql = "SELECT hoadon.id_hoa_don, hoadon.tong_tien, hoadon.hoadon.so_luong, hoadon.ngay_khoi_tao, hoadon.id_KM, thanhtoan.kieu_thanh_toan, taikhoan.ten_tk, taikhoan.dia_chi, taikhoan.sdt
    FROM `hoadon` 
...
function loadAll_lich_su_dh(){
    ...
    INNER JOIN taikhoan ON hoadon.id_tk = taikhoan.id_tk WHERE 1";
    $list_history = loadAll_lich_su_dh($sql);
    return $list_history;
}